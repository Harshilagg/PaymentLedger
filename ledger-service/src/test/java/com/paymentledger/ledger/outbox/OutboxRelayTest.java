...
class OutboxRelayTest {
    ...
    void publishesEachUnpublishedEventAndMarksItPublishedOnSuccess() {
        OutboxEvent event = new OutboxEvent(UUID.randomUUID(), "TRANSACTION_POSTED", "{}");
    ...
    void oneFailingEventDoesNotStopTheRestOfTheBatchFromPublishing() {
        OutboxEvent failing = new OutboxEvent(UUID.randomUUID(), "TRANSACTION_POSTED", "{\"a\":1}");
        OutboxEvent succeeding = new OutboxEvent(UUID.randomUUID(), "TRANSACTION_FAILED", "{\"a\":2}");
        when(outboxEventRepository.findByPublishedFalseOrderByCreatedAtAsc(any(Limit.class)))
